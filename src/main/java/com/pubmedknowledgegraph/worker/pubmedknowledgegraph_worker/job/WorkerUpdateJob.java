...
@Configuration
public class WorkerUpdateJob {

    @Value("${data.folder.update}")
    private String pubMedFolder = "PubMed/Update/";

    private final WebApplicationContext context;
    private final RemotePartitioningWorkerStepBuilderFactory remotePartitioningWorkerStepBuilderFactory;

    @Autowired
    public WorkerUpdateJob(WebApplicationContext context, RemotePartitioningWorkerStepBuilderFactory remotePartitioningWorkerStepBuilderFactory) {
    ...
    @Bean(name = "workerUpdateStep")
    public Step workerStep() {
        return this.remotePartitioningWorkerStepBuilderFactory.get("workerUpdateStep")
    ...
    public Tasklet task(@Value("#{stepExecutionContext[fileName]}") String fileName) {
        return (stepContribution, chunkContext) -> {
            ...
            if (!file.exists()) {
                String check = util.downloadUpdate(pubMedFolder, fileName);
                if (check.equals("")) {
            ...
            parseAnnotateService.setPubMedFile(true, file);
